...
[![Version](https://img.shields.io/badge/version-0.72.0-blue)](https://github.com/mouseart2025/AI-Reader-V2)
...
| 平台 | 下载 | 架构 |
|------|------|------|
| macOS | [AI Reader_0.72.0_aarch64.dmg](https://github.com/mouseart2025/AI-Reader-V2/releases/download/v0.72.0/AI.Reader_0.72.0_aarch64.dmg) | Apple Silicon (M1/M2/M3/M4) |
| Windows | [AI Reader_0.72.0_x64-setup.exe](https://github.com/mouseart2025/AI-Reader-V2/releases/download/v0.72.0/AI.Reader_0.72.0_x64-setup.exe) | x86_64 |
...
人物、地点、事件等通用类型仍是稳定的核心结构；作品特有的结构不再强塞进
固定分类，而是保存为有版本、可审核的“本书结构（Book Ontology）”。例如，
模型可以从一段带有明确总数、分组标题和重复字段的名册中发现一个集合，并为
它提出字段定义，后续章节再按已批准的定义继续填充成员。

这不是每章额外调用一次模型：逐章事实抽取只提交轻量候选信号，本地 Scout
检测到强结构证据时才触发“发现结构 → 填充集合”两阶段任务。所有候选先经过
总数、唯一性、正文证据和必填字段等确定性校验；未通过的候选保持待审核，不能
自动改变正式结构。用户可通过 `/api/novels/{novel_id}/ontology` 接口查看版本、
批准或拒绝候选，并检查集合成员。设置 `BOOK_ONTOLOGY_ENABLED=false` 可关闭该功能；
关闭 `AUXILIARY_LLM_ENABLED` 时也不会触发额外的结构发现请求。

设计、接口、Prompt 边界与《水浒传》108 人名册的可复现实验见
[自适应本书结构设计](ADAPTIVE_BOOK_ONTOLOGY.md)。
